  ...
  return (
    <Section id="services">
          ...
          {list.map((service) => (
            <StaggerItem key={service.slug}>
              <motion.button
                ...
                className="group h-full w-full scroll-mt-32 rounded-3xl border border-border bg-card p-6 text-left shadow-soft transition-all duration-300 hover:border-gold/40 hover:shadow-lift"
              >
                <div className="flex items-start justify-between">
                  <div className="rounded-2xl border border-border bg-surface p-2.5 text-gold">
      ...
      <Modal open={selected !== null} onClose={() => setSelected(null)}>
        {selected && (
          <div>
            <div className="mb-5 inline-flex rounded-2xl border border-border bg-surface p-3 text-gold">
